    ...
    rows.slice(1).forEach(row => {
        ...
        columns.forEach((col, index) => {
            ...
            if (headers[index] === 'Open-Weight') {
                ...
                badge.className = col === "Yes" ? 'badge badge-yes' : 'badge badge-no';
...
async function loadTableData() {
    try {
        ...
        const [llmModels, vlmModels, t2iModels] = await Promise.all([
            loadCSV('trustgen/data/llm.csv'),
            loadCSV('trustgen/data/vlm.csv'),
            loadCSV('trustgen/data/t2i.csv'),

        ]);
        const [llmResults, vlmResults, t2iResults] = await Promise.all([
            loadCSV('trustgen/data/llm-res.csv'),
            loadCSV('trustgen/data/vlm-res.csv'),
            loadCSV('trustgen/data/t2i-res.csv'),
        ]);
